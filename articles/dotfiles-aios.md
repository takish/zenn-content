...
ここにAI OS as Codeの自己進化ループが表れています。CLAUDE.mdもスキルのSKILL.mdもMarkdownファイルなので、Claude Code自身が読み書きできます。たとえば、新しいスキルを作ったらCLAUDE.mdのルーティングテーブルに追記する。ワークフローを改善したらチェーン定義を更新する。これらの作業をClaude Code自身に任せることで、AIエージェントが自分の動作定義を改善し続けるサイクルが成立します。従来のIaCでは、Terraformの `.tf` ファイルを書き換えるのは常に人間でした。AI OS as Codeでは、エージェント自身がコードを改善できる点が決定的に異なります。
...
Infrastructure as Codeでは、`.tf` ファイルを改善するのは常に人間でした。AI OS as Codeでは、dotfilesリポジトリ内のスキル、CLAUDE.md、フック、ワークフローチェインをAIエージェント自身が改善できます。使えば使うほど環境が育つ。これが従来のdotfiles管理やIaCとの決定的な違いです。

...
chezmoiの入門記事は日本語でも充実していますが、`modify_` プレフィックスに特化した解説はほぼ見当たりません。この記事では、`modify_` プレフィックス（ファイルを丸ごと置換せず、変換して書き出す仕組み）を使って「必要なキーだけマージする」パターンを解説します。Claude Codeの `~/.claude.json` を題材にしていますが、VS Codeの `settings.json` やSlack Desktopの設定など、アプリケーションが常時書き込みを行うJSONファイル全般に応用可能です。

jqによるJSON部分マージの実装、末尾改行の罠、テンプレートとの組み合わせまで、実運用で得た知見をまとめました。
...
Claude Codeは動作中に `~/.claude.json` へ頻繁に書き込みを行います。Claude Codeが書き込み中に `chezmoi apply` が走ると、最新の変更が失われる可能性があります。

ただし、実際のリスクはどの程度なのか。chezmoiは内部的に一時ファイルへの書き込みとrenameによるアトミックな置換を行うため、ファイルが中途半端な状態で壊れることはありません。問題になるのは「chezmoiがターゲットを読み取ったあと、書き戻すまでの間にClaude Codeが書き込んだ変更が失われる」というケースです。後述するmodify_スクリプトを使えば、影響範囲が `mcpServers` キーに限定されるため、仮にレースが発生しても失われるのはMCP設定の一時的な変更のみです。Claude Codeが `mcpServers` を書き換える頻度は低いので、実運用上の問題はほぼ起きていません。
...
`modify_` プレフィックスを使うと、このモデルが変わります。chezmoiは `modify_` プレフィックスのファイルをスクリプトとして実行し、現在のターゲット内容をstdinで受け取り、変換した結果をstdoutに出力させます。
...
`dot_claude.json.mcpServers.tmpl` の中身は、管理したいMCPサーバー定義をJSON形式で記述したものです。以下はサンプルです（APIキーやトークンを含む設定は1Password CLI等で注入する前提）。

```json
{
  "obsidian-mcp": {
    "command": "npx",
    "args": ["-y", "@anthropic/obsidian-mcp@0.3.1"],
    "env": {
      "OBSIDIAN_VAULT_PATH": "{{ .chezmoi.homeDir }}/Documents/Obsidian"
    }
  },
  "chrome-devtools": {
    "command": "npx",
    "args": ["-y", "@anthropic/chrome-devtools-mcp@0.1.5", "--port=9222"]
  },
  "github": {
    "command": "npx",
    "args": ["-y", "@modelcontextprotocol/server-github@0.6.2"],
    "env": {
      "GITHUB_PERSONAL_ACCESS_TOKEN": "{{ onepasswordRead "op://Private/GitHub PAT/token" }}"
    }
  }
}
```

`{{ .chezmoi.homeDir }}` はchezmoiのテンプレート変数で、実行環境のホームディレクトリに展開されます。`{{ onepasswordRead "..." }}` は1Password CLIからシークレットを取得する関数です。テンプレートを使わない場合は、拡張子を `.tmpl` なしにして素のJSONを書くだけで動作します。

### マージスクリプトの全コード

`modify_dot_claude.json` の全コードは15行程度です。まずは全体像を確認してください。
...
:::details 各行のポイント（クリックで展開）

**エラーハンドリング: `set -eo pipefail`**

エラーが発生したら即座にスクリプトを終了します。jqが見つからない場合やJSONパースに失敗した場合、不正な出力でファイルを壊すことを防ぎます。`pipefail` を付けることで、パイプライン内のいずれかのコマンドが失敗した場合もスクリプトを停止します。`set -e` だけではパイプの最後のコマンドの終了コードしか評価されません。

**stdin読み込み: `current=$(cat)`**

stdinの内容（現在の `~/.claude.json`）を変数に読み込みます。パイプで直接渡さず変数に格納することで、複数回参照できるようにしています。`~/.claude.json` が存在しない初回実行時は空文字列になります。

**空入力ガード: `[[ -z "$current" ]]`**

stdinが空（初回セットアップ時）の場合のガードです。空の入力をjqに渡すとパースエラーになるため、そのまま空文字列を返します。Claude Codeを起動して `~/.claude.json` が生成されたあとに再度 `chezmoi apply` を実行すればマージされます。

**安全な出力: `printf '%s\n' "$current" | jq ...`**

`echo` ではなく `printf` を使います。`echo` はシェルやプラットフォームによって `-n` や `-e` で始まる文字列をフラグとして解釈する場合があり、JSONの内容次第では意図しない動作を引き起こす可能性があります。`printf '%s\n'` はより安全でポータブルな選択肢です。

**jqによるマージ: `jq -j --argjson mcp "$mcp_servers" '.mcpServers = $mcp'`**

jq（JSONを操作するコマンドラインツール）の核心部分です。`--argjson` でMCP設定をJSON変数として渡し、`.mcpServers` キーだけを上書きします。`-j` フラグは末尾の改行を抑制します（理由は後述）。

なお、`--argjson` はコマンドライン引数として値を渡すため、プロセスリストに一時的に露出します。MCP設定に秘密情報を含む場合は、jq 1.7以降の `--argjsonfile` でファイル経由で渡す方法が安全です。

```bash
# --argjsonfile を使う場合（秘密情報がプロセスリストに露出しない）
printf '%s\n' "$current" | jq -j --argjsonfile mcp "$MCP_CONFIG" '.mcpServers = $mcp'
```

**フォールバック: `printf '%s' "$current"`**

MCP設定ファイルが存在しない場合のフォールバックです。現在の内容をそのまま返すことで、ファイルを変更しません。
...
Claude Codeは `~/.claude.json` を末尾改行なしで保存します。一方、jqはデフォルトで出力の末尾に改行を追加します。たった1バイトの違いですが、`chezmoi diff` を実行するたびに差分が表示され続けます。
...
Claude Codeの `~/.claude.json` を題材にしましたが、このパターンの本質は「アプリケーションが常時書き込むファイルの一部だけをdotfilesで管理する」という汎用的な課題への解決策です。VS Codeの設定、エディタフォークの設定、AIツールの設定ファイル。`chezmoi add` できないと諦めていたファイルがあれば、modify_スクリプトで安全にdotfiles管理に組み込めます。
